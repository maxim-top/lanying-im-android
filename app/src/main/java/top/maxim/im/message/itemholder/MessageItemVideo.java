...
public class MessageItemVideo extends MessageItemBaseView {
    ...
    private void showVideo() {
        ...
        LayoutParams imgLayoutParams = null;
        ...
        if (!TextUtils.isEmpty(imgWidth) && !TextUtils.isEmpty(imgHeight)) {
            ...
            if (w > h) {
                ...
                if (diff < limitDiff) {
                    ...
                    imgLayoutParams = new LayoutParams((int)maxLength, (int)(maxLength / diff));
                } else {
                    // 宽高比大于最大宽高比 按最大宽高比展示
                    imgLayoutParams = new LayoutParams((int)maxLength, (int)minLength);
                }
            } else if (w == h) {
                imgLayoutParams = new LayoutParams((int)maxLength, (int)maxLength);
            } else {
                diff = h / w;
                if (diff < limitDiff) {
                    // 宽高比不超过标准宽高比 小的一边按比例展示 大的一边标准长
                    imgLayoutParams = new LayoutParams((int)(maxLength / diff), (int)maxLength);
                } else {
                    // 宽高比大于最大宽高比 按最大宽高比展示
                    imgLayoutParams = new LayoutParams((int)minLength, (int)maxLength);
                }
            }
        } else {
            // 宽高比超过标准宽高比 按标准展示
            imgLayoutParams = new LayoutParams((int)maxLength, (int)(maxLength * 3 / 4));
        }
