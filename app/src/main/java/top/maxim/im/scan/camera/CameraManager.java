...
public final class CameraManager {
    ...
    /** 绘图外框最大宽度 */
    private static final int MAX_FRAME_WIDTH = 1600; // = 5/8 * 2560

    /** 绘图外框最大高度 */
    private static final int MAX_FRAME_HEIGHT = 900; // = 5/8 * 1440
    ...
    public synchronized void openDriver(SurfaceHolder holder, int degrees) throws IOException {
        ...
        setCameraDisplayOrientation(degrees, requestedCameraId >= 0 ? requestedCameraId : 0);
    ...
    public void setCameraDisplayOrientation(int degrees, int cameraId) {
    ...
    private static int findDesiredDimensionInRange(int resolution, int hardMin, int hardMax) {
        int dim = 2 * resolution / 3; // Target 3/4 of each dimension
        if (dim < hardMin) {
            return hardMin;
        }
        if (dim > hardMax) {
    ...
    public synchronized Rect getFramingRectInPreview() {
        if (framingRectInPreview == null) {
            ...
            rect.left = rect.left * cameraResolution.y / screenResolution.x
                    - context.getResources().getDimensionPixelSize(R.dimen.scan_left_right);
            rect.right = rect.right * cameraResolution.y / screenResolution.x
                    + context.getResources().getDimensionPixelSize(R.dimen.scan_left_right);
            rect.top = rect.top * cameraResolution.x / screenResolution.y
                    - context.getResources().getDimensionPixelSize(R.dimen.scan_top_bottom);
